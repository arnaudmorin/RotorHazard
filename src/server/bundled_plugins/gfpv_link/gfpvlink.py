...
class GFPVLink():
    ...
    def startup(self, args):
        ...
        if not self.enabled:
            self.logger.warning("GFPV Link is disabled. Please enable at Format tab")
    ...
    def init_ui(self, args):
        ...
        ui = self._rhapi.ui
        # Add our panel under format
        ui.register_panel("gfpv-link", "GFPV Link", "format")

        ui.register_markdown("gfpv-link", "gfpv-setup-link",
            '<a href="https://link.gfpv.fr/" class="button-like" style="display:inline-block;margin:4px 0;" target="_blank">'
    ...
    def cache_ready(self, args):
        ...
        if self.all_good():
            ...
            if not results or 'heats_by_class' not in results:
                return
            ...
            for heat in results['heats'].values():
                ...
                for rnd in heat['rounds']:
                    ...
                    for node in rnd['nodes']:
                        ...
                        if self.send_data(data):
                            self.logger.info("GFPV Link round sent OK ({})".format(node['callsign']))
                        else:
                            self.logger.warning("GFPV Link round sent FAILED ({})".format(node['callsign']))
            ...
            if class_results and 'ranking' in class_results and class_results['ranking']:
                ranking = class_results['ranking']
                if 'meta' in ranking and 'method_label' in ranking['meta']:
                    method_label = ranking['meta']['method_label']
                    if 'ranking' in ranking:
                        if method_label == 'Best 3 Laps':
                            for pilot in ranking['ranking']:
                                if pilot and 'callsign' in pilot and pilot['callsign']:
                                    pilots.append([pilot['callsign'], pilot.get('avg_time_laps', 0)])
                        elif method_label == 'Best 3 Laps (Consecutives)':
                            for pilot in ranking['ranking']:
                                if pilot and 'callsign' in pilot and pilot['callsign']:
                                    pilots.append([pilot['callsign'], pilot.get('time', 0)])
                        else:
                            # The FAI / Bracket ranking is giving a position
                            for pilot in ranking['ranking']:
                                if pilot and 'callsign' in pilot and pilot['callsign']:
                                    pilots.append([pilot['callsign'], pilot.get('position', 0)])
            #else:
    ...
    def send_data(self, data):
        ...
        try:
            x = requests.post(self.endpoint+"/push", json=data, timeout=15)
            return x.status_code == requests.codes.ok
    ...
    def is_connected(self):
        ...
        try:
            ...
            if "version" in x and self.version != x["version"]:
                needupdate = True
            return (True, needupdate)
        except (requests.ConnectionError, json.JSONDecodeError, KeyError):
    ...
    def get_heat_pilots(self, heat):
        ...
        for slot in self._rhapi.db.slots_by_heat(heat.id):
            ...
            if pilot:
                ...
                if heat.status != 0 and slot.node_index is not None:
